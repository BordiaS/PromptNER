...
def load_tweetnerd(split="validation"):
    dset = load_dataset("socialmediaie/TweetNERD")[split]
    
    columns = ["text", "entities", "types", "exact_types"]
    tweetnerd_tag_map = {0: "O",
                    1: "B-ENT",
                    2: "I-ENT",
                    }

    def get_ner_tags(data):
        tokenized_text = data['full_text'].split()
        ner_tags =[0]*len(tokenized_text)
        types={}
        phrase_entities =[]
        for i in range(len(data['start'])) :
            ...
            try:
                if  data['score'][i] > 0:
                    if start !=0:
                        token_start = len(data['full_text'][:start-1].split( ))
                    else:
                        token_start = len(data['full_text'][:start].split( ))
                    token_end = len(data['full_text'][:end].split()) 
                
                    ner_tags[token_end-1] = 2
                    ner_tags[token_start] =  1
                
                    for k in range(token_start+1, token_end-1):
                        ner_tags[k] = 2

                phrase_entities.append(data['phrase'][i])
                types[data['phrase'][i]]= "untyped"
                
            except:
                pass
        ner_tags = [tweetnerd_tag_map[k] for k in ner_tags]
        return [data['full_text'],phrase_entities,types , ner_tags   ]

    dset=[get_ner_tags(data) for data in dset]
