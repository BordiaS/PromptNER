...
def eval_tweetnerd(model, algorithm, n_runs=2, sleep_between_queries=None, limit=None, exemplar=True, coT=True,
                        defn=True, tf=True, autogen=True, **kwargs):
    splits=['ood', 'academic', 'test']
    confs = [TweetNerdOOOConfig(),TweetNerdAcademicConfig(),TweetNerdTestConfig()]
    split = kwargs.get("add_info")
    assert split in splits
    i = splits.index(split)
    config = confs[i]
    ...
    if autogen:
        ...
        config.autogenerate_annotations(algorithm, texts, tokens, labels)
    config.set_config(algorithm, exemplar=exemplar, coT=coT, defn=defn, tf=tf)
    tweetnerd = load_tweetnerd(split)
    return complete_eval(tweetnerd, model, algorithm, n_runs=n_runs, sleep_between_queries=sleep_between_queries,
                         limit=limit)
...
def run(dataset="conll", subdataset=None, gpt=True, exemplar=True, coT=True, defn=True, tf=True, name_meta=""):
    ...
    if dataset == "conll":
        ...
        eval_fn = eval_fabner
    elif dataset == "tweetnerd":
        eval_fn = eval_tweetnerd
...
def run_all_datasets(gpt=False, exemplar=True, coT=True, defn=True, tf=True,
    ...
    datasets = ["tweetnerd","conll"]
    subdatasets = {"tweetnerd": ['ood', 'academic', 'test']}
